...
const Navbar = ({ onLoginClick, onLogoClick, userRole, setView, onLogout }) => {
    ...
    <nav className="w-full px-8 py-5 flex justify-between items-center bg-[var(--bg-surface)] border-b border-[var(--border-color)] shadow-sm transition-all duration-300 sticky top-0 z-50">
      ...
      >
        <span className="text-2xl font-black text-[var(--accent)] tracking-tighter transition-colors">
          Perros{" "}
          <span className="text-[var(--color-terracotta)] group-hover:text-[#c4664a]">
            &
          </span>{" "}
          Gatos
        </span>
      ...
      <div className="flex items-center gap-5">
        ...
        >
          {isDarkMode ? (
            <Sun
              size={20}
              className="text-[var(--color-peach)] group-hover:rotate-45 transition-transform"
            />
          ) : (
            <Moon
              size={20}
        ...
        {/* Si no hay rol, mostramos login.Si hay mostramos avatar y botón de logout */}
        {!userRole ? (
          <button
